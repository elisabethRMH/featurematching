...
def fc_withweights(x, num_in, num_out, name, relu=True, reuse=False):
    #with tf.device('/gpu:0'), tf.variable_scope(name) as scope:
    with tf.variable_scope(name, reuse=reuse) as scope:
        # Create tf variables for the weights and biases
        weights = tf.get_variable('weights', shape=[num_in, num_out])
        biases = tf.get_variable('biases', [num_out])
        params = tf.concat([tf.reshape(weights, shape=[num_in*num_out]),biases],axis=0)
...
def bidirectional_recurrent_layer_bn_new(nhidden, nlayer, seq_len=1, is_training=False, input_keep_prob=1.0, output_keep_prob=1.0):
    if (nlayer == 1):
        ...
        # input & output dropout #E voor en na de fw en bw layer
...
def bidirectional_recurrent_layer_output_new(fw_cell, bw_cell, input_layer, sequence_len, scope=None):
    ((fw_outputs, # output: size ?, 29, 64 (batchsize, nb time frames, nb features at output)
      bw_outputs),
     (fw_state, #forward final state: size (?,64):c (h in paper),(?,64): h (r*h in paper),(?,1): step nb
...
def attention(inputs, attention_size, time_major=False):
    ...
    # u_omega = tf.Variable(tf.random_normal([attention_size], stddev=0.1)) #Watt
    W_omega = tf.get_variable('Variable', initializer=tf.random_normal([hidden_size, attention_size], stddev=0.1))
    b_omega = tf.get_variable('Variable_1',initializer=tf.random_normal([attention_size], stddev=0.1))
    u_omega = tf.get_variable('Variable_2',initializer=tf.random_normal([attention_size], stddev=0.1)) #Watt


    v = tf.tanh(tf.matmul(tf.reshape(inputs, [-1, hidden_size]), W_omega) + tf.reshape(b_omega, [1, -1])) #at
    vu = tf.matmul(v, tf.reshape(u_omega, [-1, 1])) #f(a): (batchsize*29 x 64 ) x (64x1) 
    exps = tf.reshape(tf.exp(vu), [-1, sequence_length]) #all the exps
    alphas = exps / tf.reshape(tf.reduce_sum(exps, 1), [-1, 1]) # all the alphas

    # Output of Bi-RNN is reduced with attention vector
    output = tf.reduce_sum(inputs * tf.reshape(alphas, [-1, sequence_length, 1]), 1)
    #E: ??? in paper, we multiply a with alphas, here it is the concatenated h_bw and h_fw (size 128 ipv 64)
    return output
