...
version of results2&3: normalization with respect to the patient group selected for training
version of results 4: normalization with respect to whole trainign dataset (41 patients)
...
for number_patients in [1]:
    di2=np.load('patient_groups.npy',allow_pickle=True)
    di2=di2.item()
    for pat_group in range(36,len(di2[number_patients])):
        ...
        train_generatorC34= SubGenFromFile(source,shuffle=True, batch_size=config.batch_size, subjects_list=train_files2, sequence_size=1, normalize=False) #TODO adapt back
...
#        train_generatorC34.datalist=boo
        ...
        test_generatorC34 =SubGenFromFile(source,shuffle=False, batch_size=config.batch_size, subjects_list=test_files, sequence_size=1, normalize=False)
        ...
        eval_generatorC34= SubGenFromFile(source,shuffle=False, batch_size=config.batch_size, subjects_list=eval_files, sequence_size=1, normalize=False)
...
#        config.out_dir1 = '/volume1/scratch/ehereman/results_adversarialDA/baselineEOG_e2earnn_1ch_losssum'#_only{:d}pat'.format(number_patients)
    ...
    #    config.out_dir2 = '/volume1/scratch/ehereman/results_featuremapping/posttrainingmap_eogtoc34_diffnet_EOGtrain{:d}pat'.format(number_patients)
        config.out_dir2 = '/esat/asterie1/scratch/ehereman/results_featuremapping/posttrainingmap_eogtoc34_diffnetTL_map{:d}pat/group{:d}'.format(number_patients, pat_group)
        config.out_dir2 = '/esat/asterie1/scratch/ehereman/results_featuremapping/posttrainingmap_eogtoc34_samec34network6_subjnorm_map{:d}pat/group{:d}'.format(number_patients, pat_group)
#        config.out_dir2 = '/esat/asterie1/scratch/ehereman/results_featuremapping/posttrainingmap_eogtoc34_samec34network4diffnorm_map{:d}pat/group{:d}'.format(number_patients, pat_group)
    ...
    #    config.evaluate_every=100
        ...
        config.training_epoch=int(10*80/number_patients)#25
...
#                    ygt = np.transpose(x_batch_c)
        ...
        #                    x_batch=np.moveaxis(x_batch,2,3)
                        ...
                        if current_step % config.evaluate_every == 0:
                            ...
                            output_loss_eval, total_loss_eval = evaluate(gen=eval_generatorC34, log_filename="eval_result_log.txt")
                            output_loss, total_loss = evaluate(gen=test_generatorC34, log_filename="test_result_log.txt")
        
                            if(output_loss_eval <= best_loss):
                                best_loss = output_loss_eval
