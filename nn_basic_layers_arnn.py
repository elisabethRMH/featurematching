'''
This code is adapted from https://github.com/pquochuy/SeqSleepNet
License: MIT © Huy Phan
...
def bidirectional_recurrent_layer(nhidden, nlayer, input_keep_prob=1.0, output_keep_prob=1.0, istrainable = True):
    fw_cell = tf.contrib.rnn.GRUCell(nhidden, trainable= istrainable)#, kernel_initializer=tf.random_uniform_initializer(-0.2, 0.2), bias_initializer=tf.random_uniform_initializer(-0.2, 0.2))
    bw_cell = tf.contrib.rnn.GRUCell(nhidden, trainable = istrainable)#, kernel_initializer=tf.random_uniform_initializer(-0.2, 0.2), bias_initializer=tf.random_uniform_initializer(-0.2, 0.2))
    if (nlayer > 1):
...
def bidirectional_recurrent_layer_new(nhidden, nlayer, input_keep_prob=1.0, output_keep_prob=1.0):
    if (nlayer == 1):
        fw_cell = tf.contrib.rnn.GRUCell(num_units=nhidden)#, kernel_initializer=tf.random_uniform_initializer)
...
def attention(inputs, attention_size, time_major=False,name='attention'):
    """
    Args:
    ...
    # Attention mechanism
    #W_omega = tf.Variable(tf.random_normal([hidden_size, attention_size], stddev=0.1), name='W')
    #b_omega = tf.Variable(tf.random_normal([attention_size], stddev=0.1), name='b')
    #u_omega = tf.Variable(tf.random_normal([attention_size], stddev=0.1), name='u')
    W_omega = tf.get_variable('W',initializer=tf.random_normal([hidden_size, attention_size],stddev=0.1)) #, minval=-0.2, maxval=0.2)) # #change Elisabeth Heremans 3/06 so that reuse can be used
    b_omega = tf.get_variable('b', initializer=tf.random_normal([attention_size], stddev=0.1))
    u_omega = tf.get_variable('u', initializer=tf.random_normal([attention_size], stddev=0.1))
