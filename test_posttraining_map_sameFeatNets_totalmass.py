...
filename="/users/sista/ehereman/Documents/code/feature_mapping/totalmass_subsets_retrainpart.mat"
...
number_patients=10
acc_matrix=np.zeros((20,4))
pat_group=0
for fold in range(20):
    
    retrain_files=files_folds['retrain_sub'][0][fold]#[0]
    ...
    retrain_generatorC34= SubGenFromFile(source,shuffle=False, batch_size=config.batch_size, subjects_list=retrain_files, sequence_size=1, normalize=normalize) #TODO adapt back
    retrain_generatorC34.normalize_per_subject()
    ...
    config.out_dir = '/esat/asterie1/scratch/ehereman/results_adversarialDA/baseline_e2earnn_1ch_losssum_subjnorm_170pat/n{:d}'.format(fold)
    dir2='/esat/asterie1/scratch/ehereman/results_featuremapping/posttrainingmap_eogtoc34_samec34network_totalmasssubjnorm_map{:d}pat'.format(number_patients)
    dir2='/esat/asterie1/scratch/ehereman/results_featuremapping/posttrainingmap_cycle_samec34network_totalmasssubjnorm_map{:d}pat'.format(number_patients)
    config.out_dir2 = os.path.join(dir2,'n{:d}/group0'.format( fold, pat_group))
#    config.out_dir2 = '/esat/asterie1/scratch/ehereman/results_featuremapping/posttrainingmap_cycle_samec34network_map{:d}pat/group{:d}'.format(number_patients,pat_group)
        ...
        with sess2.as_default():
            ...
            def dev_step(x_batch, y_batch):
                ...
                output_loss, total_loss, yhat = sess2.run(
                       [net.output_loss_mean, net.loss, net.outputlayer], feed_dict)
    ...
    #                with sess1.as_default():
            ...
            print('retrain')
            featf4, feat4, x4, ygt4, yyhatc344, yyhateog4, yyhateogmapped4 = evaluate(gen=retrain_generatorC34)
            
            kap_matrix[fold,:]=np.array((kap(ygt,yyhateog),kap(ygt3,yyhateog3),kap(ygt2,yyhateog2),kap(ygt4, yyhateog4)))
            acc_matrix[fold,:]=np.array((accuracy_score(ygt,yyhateogmapped),accuracy_score(ygt3,yyhateogmapped3),accuracy_score(ygt2,yyhateogmapped2),accuracy_score(ygt4,yyhateogmapped4)))
            x_eog= np.transpose(np.concatenate([x, x2, x3, x4],1))
            feat_c= np.transpose(np.concatenate([feat, feat2, feat3, feat4],1))
            feat_cf = np.transpose(np.concatenate([featf,featf2,featf3, featf4],1))
            ygt_c= np.concatenate([ygt, ygt2, ygt3, ygt4])+1
            yyhatc34_c= np.concatenate([yyhatc34, yyhatc342, yyhatc343, yyhatc344])+1
            yyhateog_c= np.concatenate([yyhateog, yyhateog2, yyhateog3, yyhateog4])+1
            yyhateogmapped_c = np.concatenate([yyhateogmapped, yyhateogmapped2, yyhateogmapped3, yyhateogmapped4])+1
...
#    
#
#train_generatorC342= SubGenFromFile(source,shuffle=False, batch_size=config.batch_size, subjects_list=retrain_files, sequence_size=1, normalize=normalize) #TODO adapt back
