'''
Testing the feature matching model with ARNN on the cEEGrid dataset.
'''
...
dim=12*int(10/number_patients)
acc_matrix=np.zeros((dim,3))
kap_matrix=np.zeros((dim,3))
acc_matrixC=np.zeros((dim,3))
kap_matrixC=np.zeros((dim,3))
ind=0
#VERSION WITH PATIENT GROUPS
for fold in range(12):
    for pat_group in range(int(10/number_patients)):
        ...
        list1= [eeg_retrain_data, eog_retrain_data]
        retrain_generator= SubGenFromFileHuy(filelist_lst=list1,shuffle=False, batch_size=config.batch_size, sequence_size=1, normalize_per_subject=True, replacepaths=replacepaths)
...
#        tf.app.flags.DEFINE_string("out_dir1", '/esat/asterie1/scratch/ehereman/results_adversarialDA/baseline_e2earnn_1ch_losssum_subjnorm_totalmass2/n{:d}'.format( fold), "Point to output directory")
        dir2= '/esat/asterie1/scratch/ehereman/results_featuremapping/ceegrid/fmandclasstraining_eogtoc34_diffnetwork_subjnorm_eog{:d}pat'.format(number_patients)
        ...
        with tf.Graph().as_default() as c34fi_graph:
            ...
            with sess1.as_default():
                ...
                best_dir1 = os.path.join(checkpoint_path, "best_model_acc")
        ...
        with tf.Graph().as_default():
            ...
            with sess.as_default():
                ...
                def evalfeatures_eog(x_batch, y_batch):
                    ...
                    output_loss, mse_loss, total_loss, yhat, yhateog, score, features1, features2 = sess.run(
                           [arnn.output_loss1, arnn.mse_loss, arnn.loss, arnn.prediction, arnn.prediction_eog, arnn.score, arnn.features1, arnn.features2], feed_dict)
                    return output_loss, mse_loss,total_loss, yhat, yhateog, score, features1, features2
                ...
                def evaluate(gen):
                    ...
                    while test_step < num_batch_per_epoch-1:
                        ...
                        output_loss_, mse_loss_, total_loss_, yhat_, yhat2, score_, features1_,features2_ = evalfeatures_eog(x_batch, y_batch)
                    ...
                    print(acc1)
                    return featC, featEOG, ygt, yhat, yyhateogmapped

                print('Test')
                feat_c1, feat_eog1, ygt1, yyhatc341, yyhateog1 = evaluate(gen=test_generator)
                print('Train')
                # feat_c2, feat_eog2, ygt2, yyhatc342, yyhateog2= evaluate(gen=retrain_generator)
                print('Eval')
                feat_c3, feat_eog3, ygt3, yyhatc343, yyhateog3 = evaluate(gen=eval_generator)
        
                savemat(os.path.join(out_path, "test_retEOG.mat"), dict(yhat = yyhateog1, acc = accuracy_score(ygt1, yyhateog1),kap=kap(ygt1, yyhateog1),
                                                                     ygt=ygt1))                
                savemat(os.path.join(out_path, "eval_retEOG.mat"), dict(yhat = yyhateog3, acc = accuracy_score(ygt3, yyhateog3),kap=kap(ygt3, yyhateog3),
                                                                     ygt=ygt3))                
