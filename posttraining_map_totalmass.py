...
for number_patients in [10]:
    for fold in range(20):
        
        train_files=files_folds['retrain_sub'][0][fold]#[0]
...
#        config.out_dir2 = '/esat/asterie1/scratch/ehereman/results_featuremapping/posttrainingmap_eogtoc34_diffnetTL_map{:d}pat/group{:d}'.format(number_patients, pat_group)
        config.out_dir2 = '/esat/asterie1/scratch/ehereman/results_featuremapping/posttrainingmap_eogtoc34_samec34network_totalmasssubjnorm_map{:d}pat/n{:d}'.format(number_patients, fold)
...
#        if not os.path.isdir(os.path.abspath(config.checkpoint_path1)): os.makedirs(os.path.abspath(config.checkpoint_path1))
        ...
        if not os.path.isdir(os.path.abspath(config.out_dir)): os.makedirs(os.path.abspath(config.out_dir))
...
#                    ygt = np.transpose(x_batch_c)
        ...
        #                    x_batch=np.moveaxis(x_batch,2,3)
                        ...
                        if current_step % config.evaluate_every == 0:
                            ...
                            output_loss_eval, total_loss = evaluate(gen=eval_generatorC34, log_filename="eval_result_log.txt")
