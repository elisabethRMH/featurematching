...
for fold in range(20):
    for pat_group in range(int(10/number_patients)):
        ...
        train_files=np.array([files_folds['retrain_sub'][0][fold][0][pat_group*number_patients:(pat_group+1)*number_patients]])
        eval_files=files_folds['eval_sub'][0][fold]#[0]
        test_files=files_folds['test_sub'][0][fold]#[0]
        ...
        config.out_dir2 = '/esat/asterie1/scratch/ehereman/results_featuremapping/posttrainingmap_cycle_samec34network_totalmasssubjnorm_map{:d}pat/n{:d}/group{:d}'.format(number_patients, fold,pat_group)
...
#                print("Model loaded")
    ...
    #            # initialize all variables
                ...
                def train_step(x_batch, y_batch):
                    ...
                    _, step, output_loss, cycle_loss, total_loss = sess2.run(
                       [train_op2, global_step2, net.output_loss_mean, net.cycle_loss_mean, net.loss],
                       feed_dict)
                    return step, output_loss, cycle_loss, total_loss
                ...
                def dev_step(x_batch, y_batch):
                    ...
                    output_loss, cycle_loss, total_loss, yhat = sess2.run(
                           [net.output_loss, net.cycle_loss, net.loss, net.outputlayer], feed_dict)
                    return np.sum(output_loss), np.sum(cycle_loss), total_loss, yhat
...
#                    ygt = np.transpose(x_batch_c)
        ...
        #                    x_batch=np.moveaxis(x_batch,2,-1)
        #                    x_batch=np.moveaxis(x_batch,2,3)
                        train_step_, train_output_loss_, train_cycle_loss_, train_total_loss_ = train_step(x_batch_eog, x_batch_c)
